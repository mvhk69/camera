...
def initialize_capture_device(device_index):
    ...
    """
    cap = cv2.VideoCapture(device_index, cv2.CAP_DSHOW)  # Use DirectShow backend for Windows
    ...
    if actual_fps < CAPTURE_FPS:
        print(f"Warning: Requested FPS ({CAPTURE_FPS}) exceeds camera capability ({actual_fps}).")
    return cap
...
def main():
    ...
    try:
        ...
        print(f"Camera {device_index} initialized at {CAPTURE_WIDTH}x{CAPTURE_HEIGHT} @ {CAPTURE_FPS} FPS.")
